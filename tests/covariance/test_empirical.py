...
import numpy as np
from precise.covariance.empirical import ecov_init,merge_ecov, ecov_update
...
def test_onlineempirical():
    ...
    for observation in data:
        ocov = ecov_update(m=ocov, x=observation)
...
def test_merging():
    ...
    ocov_part1 = ecov_init(n_dim=3)
    ocov_part2 = ecov_init(n_dim=3)
    ocov_both = ecov_init(n_dim=3)

    for row in data_part1:
        ocov_part1 = ecov_update(m=ocov_part1, x=row)
        ocov_both = ecov_update(m=ocov_both, x=row)

    for row in data_part2:
        ocov_part2 = ecov_update(m=ocov_part2, x=row)
        ocov_both = ecov_update(m=ocov_both, x=row)

    ocov_merged = merge_ecov(s=ocov_part1, other_s=ocov_part2)
