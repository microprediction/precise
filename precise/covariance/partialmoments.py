...
# Experimental estimator inspired by NNS package 


def pm_ema_scov(s:dict, x=None, r=0.025)->dict:
    if s.get('s_c') is None:
        if isinstance(x,int):
            return _pm_ema_scov_init(n_dim=x, r=r)
        else:
            s = _pm_ema_scov_init(n_dim=len(x), r=r)
    if x is not None:
        s = _pm_ema_scov_update(s=s, x=x, r=r)
    return s


def _pm_ema_scov_init(n_dim, r):
