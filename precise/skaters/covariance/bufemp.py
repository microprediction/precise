...
def buf_emp_pcov_d0_n20(y:Y_DATA_TYPE, s:dict, k=1):
    assert k==1
    return buf_emp_pcov_d0_factory(y=y, s=s, n_buffer = 20)


def buf_emp_pcov_d0_n50(y:Y_DATA_TYPE, s:dict, k=1):
    assert k==1
    return buf_emp_pcov_d0_factory(y=y, s=s, n_buffer = 50)


def buf_emp_pcov_d0_n100(y:Y_DATA_TYPE, s:dict, k=1):
    return buf_emp_pcov_d0_factory(y=y, s=s, n_buffer = 100)


def buf_emp_pcov_d1_n20(y:Y_DATA_TYPE, s:dict, k=1):
    assert k==1
    return buf_emp_pcov_d1(y=y, s=s, n_buffer = 20)


def buf_emp_pcov_d1_n100(y:Y_DATA_TYPE, s:dict, k=1):
    assert k==1
    return buf_emp_pcov_d1(y=y, s=s, n_buffer = 100)


def buf_emp_pcov_d1(y:Y_DATA_TYPE, s:dict, n_buffer:int=100, k=1):
    """
        For when changes are iid gaussian
    """
    assert k==1
    return d1_factory(f = buf_emp_pcov_d0_factory, y=y, s=s, n_buffer=n_buffer)


def buf_emp_pcov_d1_long_form(y:Y_DATA_TYPE, s:dict, n_buffer:int=100, k=1):
