...
def diagonal_portfolio_variance(cov=None, pre=None, ridge=0.1):
    """
        Variance of the unit min-var portfolio
        (Used in some hierarchical methods to allocate capital)
    """
    w = diagonal_portfolio_factory(pre=pre, cov=cov, ridge=ridge)
...
def diagonal_allocation_factory(covs:List, pres:List=None, ridge=0.1)->[float]:
    ...
    return normalize([ 1/diagonal_portfolio_variance(cov=cov, pre=pre, ridge=ridge) for cov, pre in zip_longest(covs, pres, fillvalue=None) ])
