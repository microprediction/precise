...
methods have been confined to batch settings. Experiments in five regimes separate what
this buys from what it costs: against exponentially weighted linear shrinkage the learned
calibration is fifteen to twenty percent better wherever the dependence structure actually changes,
and worse wherever it does not---by a factor of twenty against an expanding-sample analytical
shrinker on a static spectrum. We also report a negative result: conditioning the learned map on the
global shape of the spectrum, which we expected to matter, does not.
\end{abstract}
...
\toprule
 & uncleaned & \multicolumn{2}{c}{linear, weighted} & nonlinear, expanding & \multicolumn{3}{c}{learned calibration} \\
\cmidrule(lr){2-2}\cmidrule(lr){3-4}\cmidrule(lr){5-5}\cmidrule(lr){6-8}
regime & raw EW & Ledoit--Wolf & OAS & analytical NLS & rank & eigenvalue & shared \\
\midrule
isotropic         & 0.340 & \textbf{0.0007} & \textbf{0.0007} & 0.0007          & 0.0026 & 0.0018          & 0.0018 \\
broad spectrum    & 0.206 & 0.204           & 0.199           & \textbf{0.0072} & 0.135  & 0.147           & 0.147  \\
factor rotation   & 0.243 & 0.238           & 0.231           & 0.206           & 0.191  & \textbf{0.186}  & 0.186  \\
factors vanish    & 0.299 & 0.127           & 0.124           & 0.239           & 0.105  & \textbf{0.103}  & 0.103  \\
regimes alternate & 0.330 & 0.131           & 0.128           & 0.217           & 0.114  & 0.109           & \textbf{0.109} \\
\bottomrule
\end{tabular}
\caption{Mean squared Frobenius error relative to the squared norm of the truth; lower is better,
best in each row in bold. Eight seeds, $p=32$, $6000$ observations each.}
\end{table}

Three things are worth reading off, one of them negative.

\emph{Learning buys adaptivity, and only adaptivity.} On the three non-stationary regimes the
calibrated estimators beat both linear shrinkers by fifteen to twenty percent---$0.186$ against
$0.231$ under a rotation, $0.103$ against $0.124$ when the factor structure dies, $0.109$ against
$0.128$ under alternation. On the two static regimes they lose, and against the expanding-sample
analytical shrinker on a broad static spectrum they lose by a factor of twenty ($0.147$ against
$0.0072$). This is the expected shape of the trade and it is worth stating plainly: the learned map
is paying, in variance, for an adaptivity that a static problem does not reward. An estimator that
does not know which regime it is in cannot have both.
...
constructed by us to separate the failure modes we suspected; there is no real equity study, and
the advantage over linear shrinkage is confined to the non-stationary regimes, which are exactly the
ones a sceptic would say we chose. The open question is not whether spectral cleaning can be made online. It plainly
